...
(async function () {
    ...
    const unidadesEco = await getUnidadesEco("/app/vacantes");
    ...
    console.log(headerMainDataIndex);
      ...
      for (const element in data) {
        if(element != 0) {
            ...
            sendVacantesButton = sendVacantesLink;

            const idVacante = data.indexOf(element);

            const sendVacantesObject = {
                 idVacante: idVacante,
                sendVacantes: sendVacantesButton
            };

            // Conseguir indice del elemento en el data

            sendVacantesArray.push(sendVacantesObject);
        }

        
    }

    goToVacantesDetails(sendVacantesArray)
